...
function AddFishProducts() {
  ...
  const loadCategories = async () => {
    try {
      const res = await fetch(
        `${process.env.REACT_APP_SELLER_API_URL}/api/seller/categories?seller_id=${seller_id}`
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let res;
      if (editingId) {
        res = await fetch(`${process.env.REACT_APP_SELLER_API_URL}/api/seller/fish/${editingId}`, {
          method: "PUT",
          body: formData,
        });
      } else {
        res = await fetch(`${process.env.REACT_APP_SELLER_API_URL}/api/seller/add-fish`, {
          method: "POST",
          body: formData,
        });
      }
