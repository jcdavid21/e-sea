  ...
  const exportToCSV = () => {
    ...
    Swal.fire({
        ...
        const rows = filteredOrders.map(order => [
            ...
            order.product_name,
            order.quantity,
            `PHP ${Number(order.price).toFixed(2)}`,
            `PHP ${(Number(order.price) * Number(order.quantity)).toFixed(2)}`,
            order.status,
            ...
            <tbody>
                ...
                currentItems.map((order, index) => (
                    ...
                    <td className="total-cell">
                        {formatPrice(Number(order.price) * Number(order.quantity))}
                    </td>
                    <td>
                ...
                <div style={styles.infoGrid}>
                  ...
                  <div style={styles.infoItem}>
                    ...
                    <span style={{...styles.infoValue, fontSize: '1.2rem', fontWeight: '700', color: '#1e3c72'}}>
                      {formatPrice(Number(selectedOrder.price) * Number(selectedOrder.quantity))}
                    </span>
                  </div>
                </div>
              </div>
