...
const ShopProductPage = () => {
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        const res = await fetch(`${process.env.REACT_APP_BUYER_API_URL}/api/shop/${shopId}/products`);
        
        if (!res.ok) {
          const allShopsRes = await fetch(`${process.env.REACT_APP_BUYER_API_URL}/api/shop`);
